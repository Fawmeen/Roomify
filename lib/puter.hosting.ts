...
import { HOSTING_CONFIG_KEY, HOSTING_DOMAIN_SUFFIX, createHostingSlug, isHostedUrl, imageUrlToPngBlob, fetchBlobFromUrl, getImageExtension, getHostedUrl} from "./utils";
...
export const uploadImageToHosting = async ({hosting, url, projectId, label}: StoreHostedImageParams): Promise<HostedAsset | null> => {
    ...
    try{
        ...
        console.log(`Failed to store host: ${e}`);
        return null;
    }
}
